...
#cv2.imwirite(저장할 파일이름(확장자 선택가능) , 저장할 영상 데이터 , 옵션(속성&값의 정수 쌍)) : 영상파일 저장하기
...
그레이스케일 영상에서는 컬러로 그리기가 되지 않기때문에 cv2.cvtColor()함수로 BGR컬러 영상으로 변환후 그리기 함수 호출
...
isColor의 기본값은 True 컬러영상을 저장하는용도임. grayscale로 저장하기위해서는 color포멧으로 변환해서 저장해야함.
